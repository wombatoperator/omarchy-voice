...
Quickshell's PATH is one of those, so the bar widget's click ends up here: the
wrapper has to behave like the CLI when it is handed a command line, and only
fall back to `status` when it is called bare as `omarchy voice`.
"""
...
class BareRouteTests(unittest.TestCase):
    ...
    def test_a_command_line_is_passed_through_not_appended_to_status(self):
        ...
        # is what the bar widget's click was getting.
    ...
    def test_a_subcommand_is_understood(self):
        # Read-only stand-in for the `listen toggle` the bar widget sends;
        # under the old wrapper this became `status log`, an argparse error.
